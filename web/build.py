...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .top{background:var(--sea-deep);color:#eef4f2;padding:13px 22px 12px;border-bottom:3px solid var(--coral)}
  ...
  .coord{font-family:var(--mono);font-size:11px;color:var(--sea);letter-spacing:.3px;display:flex;gap:7px;align-items:center}
  .coord .nogeo{color:var(--muted)}
  .title{font-family:var(--disp);font-weight:600;font-size:16.5px;line-height:1.22;margin:5px 0 7px}
  .specs{font-size:13px;color:var(--muted);display:flex;gap:13px;flex-wrap:wrap}
  .amen{margin-top:8px;font-size:15px;letter-spacing:3px;line-height:1}
  .foot{display:flex;justify-content:space-between;align-items:baseline;margin-top:11px;gap:10px}
  .price{font-family:var(--mono);font-weight:700;font-size:16px;color:var(--coral-d)}
...
function render(first){
  ...
  view.forEach((d)=>{
    ...
    const coord=(d.lat!=null
      ? '<span>'+d.district+'</span><span>'+d.lat.toFixed(3)+', '+d.lng.toFixed(3)+'</span>'
      : '<span class="nogeo">📍 позиция не указана</span>')+fb;
    ...
    card.innerHTML='<div class="coord">'+coord+'</div>'+
      '<div class="title">'+d.title+draft+'</div>'+
      '<div class="specs"><span>🛏 '+(d.bedrooms??'—')+'</span><span>🏠 '+(TYPE[d.type]||d.type)+'</span></div>'+
      (amen(d.amenities)?'<div class="amen">'+amen(d.amenities)+'</div>':'')+ref+
    ...
    if(d.lat!=null){
      ...
      const m=L.marker([d.lat,d.lng],{icon}).addTo(map)
        .bindPopup('<b>'+d.title+'</b><br>'+(d.price!=null?priceFull(d.price)+' ฿'+unit:'цена не указана')+(d.sources.length>1?'<br>'+d.sources.length+' источника':'')+'<br><a href="'+firstUrl(d)+'" target="_blank">оригинал ↗</a>');
      markers[key]=m;
